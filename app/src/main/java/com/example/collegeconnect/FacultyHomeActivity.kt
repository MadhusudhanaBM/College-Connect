...
import kotlinx.android.synthetic.main.fac_latest_message_row.view.new_msg_img
import kotlinx.android.synthetic.main.fac_latest_message_row.view.new_msg_text
import kotlinx.android.synthetic.main.fac_latest_message_row.view.text_view_for_id
...
class FacultyHomeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigationView.setNavigationItemSelectedListener {
            when (it.itemId) {
                R.id.itemab -> {
                    var i=Intent(applicationContext,About_Activity::class.java)
                    startActivity(i)
        ...
        try {
            mFirebaseDatabaseInstances?.collection("Faculty")!!.document(username.toString()).get().addOnSuccessListener { result ->
                ...
                if (fac == null) {
                    ...
                    FID = fac.FID
                    FBr = fac.branch
                    val query: Query = FirebaseFirestore.getInstance().collection("LatestMessage")?.whereArrayContains("id",FID)?.orderBy("timestamp",Query.Direction.DESCENDING)
