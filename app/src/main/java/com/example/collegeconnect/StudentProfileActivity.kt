...
class StudentProfileActivity : AppCompatActivity() {
    ...
    fun addUploadRecordToDb(uri: String){
        ...
        url = uri
        try{
            mFirebaseDatabaseInstances?.collection("Student")!!.document(username).get().addOnSuccessListener {
                    result ->
                val stud=result.toObject(Student::class.java)
                if(stud==null){
                    Toast.makeText(this, "ERROR IN LOADING THE DATA", Toast.LENGTH_LONG).show()
                    ...
                    var student = Student(stud.name,stud.usn,stud.branch,stud.year, stud.sec,stud.email, stud.mobile, stud.address, stud.password,url,stud.label1,stud.label2)
                    mFirebaseDatabaseInstances?.collection("Student")?.document(username!!)?.set(student)
